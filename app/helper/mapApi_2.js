...
var mapOpt = {
    ...
    center: new google.maps.LatLng([datat[3].lat], [datat[1].lng]),
...
projMapOpt = {
    zoom: 11,
    scrollwheel: false,
    center: new google.maps.LatLng(datat[0].proje[0].lat, datat[0].proje[0].lng),
    mapTypeId: google.maps.MapTypeId.ROADMAP,
    styles: stls
};
...
var markers = [];
var infowindow;
var satisiDevamEdenProj = datat.filter(function (proje) { return proje.c_id == "1" });
var tamamlananProj = datat.filter(function (proje) { return proje.c_id == "2" });
var gelecekProj = datat.filter(function (proje) { return proje.c_id == "3" });
var defaultProj = [datat[0]];
...
runMapCat('.hepsi-harbtn', datat, map);
